...
public class AnthropicExplanationService implements ExplanationService {

    private static final Logger log = LoggerFactory.getLogger(AnthropicExplanationService.class);

    private final ExplanationService fallback;
    private final HttpClient httpClient;
    private final AppConfig appConfig;

    public AnthropicExplanationService(ExplanationService fallback) {
    ...
    private String explain(String prompt, String fallbackText) {
        ...
        String baseUrl = trimToNull(appConfig.getAiBaseUrl());
        String authToken = trimToNull(appConfig.getAiAuthToken());
        String model = trimToNull(appConfig.getAiModel());
        if (baseUrl == null || authToken == null || model == null) {
            return fallbackText;
        }
        ...
        try {
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                ...
                log.info("Remote explanation request failed with status {}", response.statusCode());
            }
        } catch (InterruptedException e) {
            log.info("Remote explanation request failed", e);
            Thread.currentThread().interrupt();
        } catch (IOException e) {
            log.info("Remote explanation request failed", e);
        } catch (IllegalArgumentException e) {
            log.info("Remote explanation URL is invalid", e);
        }
        return fallbackText;
    }
